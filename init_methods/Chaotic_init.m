function [population] = Chaotic_init(population_size, bound_dict, map)
% CHAOTIC_INIT Create an initial population using chaotic sampling.
%
% This function generates a population of candidate solutions for an
% optimization algorithm using a chaotic map. The chaotic sequence is used
% to select parameter values from the allowed search space, providing
% better diversity than purely random sampling.
%
% Inputs:
%   population_size
...
%
%   map
%       Name of the chaotic map used to generate the sampling sequence.
%
%       Supported options:
%           "Tent"
%           "Sine"
...
%
%   population = Chaotic_init(100, bounds, "Tent");
%
% Coded by::
...
% ---------------------------------------------------------------------
    ...
    % The selected chaotic map generates values that
    % are later used to choose parameter values.
    ...
    % using the specified minimum, maximum, and step size.
    ...
    % For each parameter, use the chaotic sequence
    % to select a value from its corresponding pool.
    for i = 1:population_size

        for j = 1:size(param_values,1)

            selected_value = ...
                paramPool{j}(ceil(C_map(i,j) * length(paramPool{j})));
...
%   5. Step size fits within the specified range.
